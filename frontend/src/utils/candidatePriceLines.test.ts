...
// tier table (unpriced / range / single), a token row reads its per-million
// prices with the cache pair as a quiet second line only when present. The
// translator stands in for vue-i18n by echoing key and named args, so the
// assertions pin WHICH string lands in the cell without duplicating locale
// copy.
