...
.NOTES
    Environment overrides (all optional):
      ...
      YOLO_MIRROR=https://host/  prefix every GitHub request with a mirror (e.g. a
                               Cloudflare proxy) for networks where GitHub is slow or
                               blocked. Trailing slash required. When set, it is also
                               handed to the service so self-update uses it.
...
# own host) so `irm <proxy>/install.ps1 | iex` routes through the mirror with no
# env var to set. Keep this line verbatim; the proxy matches it literally.
$MIRROR = if ($env:YOLO_MIRROR) { $env:YOLO_MIRROR } else { '' }
...
# ---------------------------------------------------------------------------
# Prefix a GitHub URL with $MIRROR when set, so a mainland install can route
# every GitHub request (version lookup, binary, checksums) through a mirror;
# a plain passthrough when $MIRROR is empty.
# ---------------------------------------------------------------------------
function Mirror-Url($url) {
    if ($MIRROR) {
        # Strip ALL trailing slashes, then join with exactly one, so any number
        # of trailing slashes on $MIRROR still yields a valid single-slash join.
        $base = $MIRROR
...
function Download-AndExtract {
    ...
    if (-not $skipVerify) {
        ...
        foreach ($line in (Get-Content $sumsPath)) {
            ...
            $name = ($parts[1].TrimStart('*') -split '[\/]')[-1]
            if ($name -eq $asset) {
                $expected = $parts[0]
                break
            }
...
function Set-RawPath($scope, $value) {
    ...
    # [Environment]::SetEnvironmentVariable(...,"Machine"/"User") stores REG_SZ,
    # which freezes %SystemRoot%-style entries into literal text and corrupts PATH.
    if ($scope -eq 'Machine') {
        $key = [Microsoft.Win32.Registry]::LocalMachine.OpenSubKey('SYSTEM\CurrentControlSet\Control\Session Manager\Environment', $true)
    } else {
        $key = [Microsoft.Win32.Registry]::CurrentUser.OpenSubKey('Environment', $true)
    }
    if (-not $key) { throw "cannot open $scope Environment registry key" }
    try {
...
function Backup-BeforeUpgrade {
    ...
    # 若上次安装中途中断（如下载失败），bin\yolorouter.exe 不会落地，
    # 但 config 可能残留，会被上面的逻辑误判为升级。此时备份要用的旧
    # 二进制不存在，实为全新安装，直接跳过备份。
    if (-not (Test-Path $binary)) {
        warn "$(m '旧二进制不存在（上次安装未完成），视为全新安装，跳过升级前备份。' 'No existing binary (previous install incomplete); treating as a fresh install and skipping the pre-upgrade backup.')"
        $script:IS_UPGRADE = $false
        return
    }
...
function Setup-ScheduledTask {
    ...
    $proxyLine = ""
    if ($MIRROR) { $proxyLine = "set `"YOLO_UPDATE_GITHUB_PROXY=$MIRROR`"`r`n" }
...
function Write-Summary {
    ...
    Write-Host "  $(m '卸载:   ' 'remove: ') & ([scriptblock]::Create((irm $installUrl))) --uninstall"
    Write-Host
