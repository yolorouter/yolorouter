...
- **多用户与 SSO** —— 团队成员通过任意 OAuth2/OIDC 身份源（Zitadel、GitHub、Keycloak 等）登录，首次登录自动建号、无需邀请。成员自助管理自己的 API Key，只能看到自己的用量与费用；管理员拥有全局视图，可按账号筛选所有统计，并可对账号做升降角色与禁用。禁用即刻生效：会话立即登出、名下所有 Key 立即失效。
