...
// UpdateRequestLogVideoSettlement back-fills the request_logs row a video
// submit wrote with the settlement digest — the cost the charge decided
// and the seconds the job actually delivered — minutes or days after the
// request itself ended. The projection that puts video bills into the
// same analytics every per-request bill already feeds, in the one
// billing unit (per second) the row then reads in. A miss (no row, or a
// task created before the linking column existed, whose request_id is
// empty) is not an error: the charge itself already landed, and the
// projection is best-effort by design.
func UpdateRequestLogVideoSettlement(db *gorm.DB, requestID string, micros int64, seconds int) error {
	if requestID == "" {
		return nil
	}
	return db.Model(&model.RequestLog{}).
		Where("request_id = ?", requestID).
		Updates(map[string]any{"cost_micros": micros, "cost_known": true, "usage_seconds": seconds}).Error
}
